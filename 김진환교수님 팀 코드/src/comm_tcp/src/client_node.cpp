...
void Listener::callback(const core_msgs::ball_position::ConstPtr& msg) {
   memset(topic_message, 0, 256);
   //strcpy(topic_message,  msg->img_x[0]);
   ROS_INFO("I heard:[%f]", msg->img_x[0]);
}
...
int main(int argc, char *argv[]) {
 ...
 Listener listener;
        ros::Subscriber client_sub = nh.subscribe("/position", 1, &Listener::callback, &listener);
