...
double yaw_init = 1.408;
double yaw_init2 = -1.56;
double yaw_now = 0.0;
double yaw_old = 0.0;
double yaw_temp= 0.0;
double no_ball = 0.0;
...
static void Calib2(){
	...
	yaw_init2 = yaw_init2 + yaw_temp / (n+0.0);
}

static void Facegreen(){
	dataInit();

	if(!green_e){
		data[4] = 0.2;
	}else if(green_r && !green_l){
		//turn right
	}else if(!green_r && green_l){
		//turn left
	}else if(green_r && green_l){
		//precise control
		if(rbl-320 > 320-lbr+10){
			//go right

			data[4] = 0.05
		}else if(320-lbr > rbl-320+10){
			// data[2] = M_PI-0.000001;
			data[3] = -0.05;
		}else if(rbl-320 > 320-lbr+4){
			data[4] = (rbl-lbr-640)*0.0005;
		}else if(320-lbr > rbl-320+4){
			data[4] = (rbl-lbr-640)*0.0005;
		}else{
			flag_error = false;
		}
...
static void FinalOri(){
	...
	dest2mean = dest2/5;
	if(dest2mean < 0.04 && dest2mean > -0.04){
		flag_error = false;
	}
	// data[4] = 0.131;
...
static void Phase2(){
	...
	if(!green_e){
		...
		if(green_l && green_r){
			if(rbl-320 > 320-lbr+mindgap){
				//go right
				data[2] = 0;
				data[3] = 0.05;
			}else if(320-lbr > rbl-320+mindgap){
				data[2] = M_PI-0.000001;
				data[3] = 0.05;
				//go left
			}
			// if(!green_ll && green_rr){
			// 	data[2] = 0.0;
			// 	data[3] = 0.05;
			//
			// }else if(green_ll && green_rr){
			// 	data[2] = M_PI-0.000001;
			// 	data[3] = 0.05;
			// }
			//proceed slowly
...
int main(int argc, char **argv) {
	...
	while ((ch = getch()) != 'q') {
		...
		if(data[21] > 0.5){
			...
			Calib2();
			printf("calib2\n");
			outfile << "CAL_AVG yaw2: "<<yaw_init2 <<endl;
			outfile << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
		}else if(data[20] >0.5){
			outfile << "~~~~~~~~~~~~CALIBRATING~~~~~~~~~~~~~~"<<endl;
			outfile << "N: " <<n << endl;
