...
double yaw_init = -1.85;
double yaw_init2 = 1.564;
double yaw_now = 0.0;
double yaw_old = 0.0;
double yaw_temp= 0.0;
double no_ball = 0.0;
...
static void Facegreen(){
	...
	if(!green_e){
		...
		if(rbl-320 > 320-lbr+10){
			...
			data[4] = -0.03;
		}else if(rbl-320 > 320-lbr+2){
			data[4] = 0.01;
		}else if(320-lbr > rbl-320+2){
			data[4] = -0.01;
		}else{
...
static void Phase2(){
	...
	if(!green_e){
		...
		if(green_l && green_r){
			...
			outfile<<"mindgap data[2],[3]: "<<data[2] << " // " << data[3]<<endl;
...
int main(int argc, char **argv) {
	...
	while ((ch = getch()) != 'q') {
		...
		if(gohome){
			...
			outfile << flag_final3 <<" "<<flag_final<< endl;
			if(flag_final3){
				data[2] = M_PI/2.0;
				data[3] = 0.3;
				data[4] = 0;
		...
		else
		{
			...
			outfile<<"the fuck"<<data[4]<<endl;
			if(flag_init){
				outfile<<"is wrong" <<data[4] <<endl;
