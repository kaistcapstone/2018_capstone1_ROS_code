...
static void Orient(){
	...
	if (yaw_now > yaw_init+RANGE_TURN){
		data[4] =turn_max*(yaw_now-yaw_init)/3.14;
	}
	else if(yaw_now < yaw_init-RANGE_TURN){
		data[4] =  turn_max*(yaw_now-yaw_init)/3.14;
	}
	else{
		data[4] = yaw_now;
...
void dataInit()
{
	...
	data[9] = 1; //GamepadTriggerLength(_dev, TRIGGER_RIGHT);
...
void autoBallTrack(const data_massage::BTC& msg)
{
	...
	// 	data[4] =0;
	// 	data[1] = (450-y)/(600.0)*(strafe_max-strafe_min)+strafe_min;
	// 	if(x<(resol_x/2 - RANGE_DEAD1)){
	// 		data[0] = 20.0 /(resol_x/2 - RANGE_DEAD1)* (x-(resol_x/2 - RANGE_DEAD1));
	// 		//move right
	// 		// data[4] = -1*turn_min +turn_max /(resol_x/2 - RANGE_DEAD1)* (x-(resol_x/2 - RANGE_DEAD1));
	//
	// 	}else if(x<(resol_x/2 + RANGE_DEAD1)){
