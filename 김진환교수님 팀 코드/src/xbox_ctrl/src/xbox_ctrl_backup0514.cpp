...
int decision[dec_buf];
memset(decision,0,dec_buf*4);
int max_element = 0;
int min_element = 1;
...
void autoBallTrack(const data_massage::BTC& msg)
{
	...
	for(int k = 0; k<dec_buf-1; k++){
		decision[k] = decision[k+1];
	}
	decision[dec_buf-1] = msg.data2;
	min_index = (min_element(decision, decision+dec_buf)-decision);
	max_index = (max_element(decision, decision+dec_buf)-decision);
	...
	if(x<(resol_x/2 - RANGE_DEAD1)){
		...
		//nothing
		data[2] = M_PI/2.0;
		data[3] = (y-450)/(450.0)*(strafe_max-strafe_min)+strafe_min;
		data[4] = 0;
		if(y>450){
			flag_sweep = true;
