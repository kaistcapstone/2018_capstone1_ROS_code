...
//     Mat frame;  //assign a memory to save the images
     ...
     msg2.size = contours_g.size();
     msg2.img_x.resize(contours_g.size());
     msg2.img_y.resize(contours_g.size());
     msg2.img_r.resize(contours_g.size());
     ...
     for( size_t i = 0; i< contours_g.size(); i++ )
        {
            ...
            if(radius_g[i] > iMin_tracking_ball_size)
            {
                ...
                msg2.img_x[i] = center_g[i].x;
                msg2.img_y[i] = center_g[i].y;

                msg2.img_r[i] = radius_g[i];
...
int main(int argc, char **argv)
{
   ...
   pubg = nh.advertise<core_msgs::ball_position>("/posg", 100);
