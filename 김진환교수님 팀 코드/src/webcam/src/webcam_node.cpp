...
int main(int argc, char** argv)
{
  ...
  cv::Mat buffer; //assign a memory to save 1mages with variable name 'frame'
  sensor_msgs::ImagePtr msg; //declare a pointer of sensor_msgs::Image.

  ros::Rate loop_rate(15); //set loop rate. you can set hz here. The maximum hz of webcam device is 30hz. If you set the hz here larger than 30, it is meaningless.
  while (nh.ok()) {
    cap >> frame;  //transfer image captured by 'cap' to 'frame'
    //이미지 데이터의 크기를 줄이기 위해 원본 이미지보다 낮은 320*240 화질로 변경함
    if(reduced==true){
	cv::resize(frame, buffer, cv::Size(160,120)); //reduced the size of the image
    }
    else{
	buffer = frame;
    }
